...
+ luaJIT的代码可阅读性非常差，许多函数都是通过工具生成的汇编代码。以下通过gbd调试一步步描述luaJIT虚拟机的执行流程，包括触发热点函数后的处理。
  ...
  + jit优化：
    ...
    + 之后会在```lj_BC_JFORI```中调用函数```lj_BC_JLOOP```，在这个函数中会根据当前traceNo在trace数组中偏移找到对应的动态优化生成的函数的起始地址。    
      ...
      ```
# TODO
  + luajit动态优化具体实现步骤？哪些情况会优化失败？
  + luajit其他模块的具体实现，比如GC，大概看了下跟原生lua的增量式GC一样。
  + 目前看来对于游戏服务器开发的应用场景基本上都可以用luajit替换lua，以便提升性能。那么原生lua相较于luajit有哪些优势？现在看到的最新版本lua有分代式GC，而luajit只有增量式GC，除此之外还有哪些优势？